...
```sh
python3 tools/build_site.py          # regenerate site/index.html + index_cards.csv
python3 tools/build_site.py --check  # verify both are current; writes nothing
```

Requires Python 3.6+, standard library only — no dependencies.

This is the only build step, and it produces two files: `site/index.html`, whose
source is `data/site_template.html`, and `data/index_cards.csv`, a flat lossy
projection of the dataset for spreadsheet use. Open `site/index.html` directly in
a browser to use the site (no server required).

The CSV was maintained by hand until it drifted — 85 `output_unit` cells stayed
empty after the JSON filled them — so it is derived now, and `--check` covers
it.
