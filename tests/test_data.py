...
class TestCsvProjection(unittest.TestCase):
    """index_cards.csv is generated by build_site.py, and --check keeps the
    committed file matching the generator. This guards the other half: that the
    generator agrees with the JSON. --check cannot see a generator bug, because
    it compares the file against the same wrong output.

    Every column, not a sample. The file was hand-maintained until it drifted --
    85 output_unit cells stayed empty after the JSON filled them, unnoticed
    because the test that guarded it compared five of its twelve columns."""
    ...
    def test_every_column_of_every_row_matches_the_json(self):
        rows = self._rows()
        self.assertEqual(len(CARDS_DATA), len(rows), "row count")
        # Paired positionally, so row ORDER is asserted too: the projection is
        # meant to be the dataset flattened, not a reordering of it.
        for card, row in zip(CARDS_DATA, rows):
            for column, got in row.items():
                want = "" if card.get(column) is None else str(card.get(column))
                self.assertEqual(want, got, f"{card['id']}.{column}")

    def test_the_projection_is_lossy_on_purpose(self):
        """The nested fields are absent by design -- flattening inputs or
        used_in would need a convention this file has no way to express. Asserted
        so nobody 'fixes' the omission by inventing one."""
        columns = set(self._rows()[0])
        for nested in ("inputs", "used_in", "growing_conditions", "livestock_yields"):
            self.assertNotIn(nested, columns)
